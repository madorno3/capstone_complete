...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://capstone_db_nwr7_user:Giawesy9uLfH7aRteF6T941lClrzDdnC@dpg-ctukthtds78s73fpf0m0-a/capstone_db_nwr7"
...
def playlist_details(playlist_id):
    ...
    flat_tracks = [item[0] for item in tracks]
    cleaned_tracks = [track.strip('"\'') for track in flat_tracks]
    cover_res = sp.playlist_cover_image(playlist_id)
    cover = cover_res[0]['url']

    print(cleaned_tracks)
    return render_template("playlist_details.html", current_playlist=current_playlist, cover=cover, tracks=tracks, user_id=user_id,username=username,cleaned_tracks=cleaned_tracks)
...
def new_playlist_details():
    ...
    if form.validate_on_submit():
        ...
        flat_tracks = [item[0] for item in playlist_tracks]
        cleaned_tracks = [track.strip('"\'') for track in flat_tracks]
